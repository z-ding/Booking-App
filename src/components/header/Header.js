...
const Header = ({ type }) => {
  //calendar
  const [opendates, setopendates] = React.useState(false) //control whether the calendar appears
  ...
  //useNavigate redirect user to other component / page
  const navigate = useNavigate()
  const [destination, setdestination] = React.useState("")
  const handleSearch = ()=>{
    navigate("/hotels", { state: { destination, date, options } }); //pass in state as an option object
   }
  return (
        ...
        </div>
        {
          type !== "list" && <>
            ...
            <p className="headerdescription">
              Get rewards by creating an account
            </p>
            ...
            <div className="headerSearch">
              ...
              <div className="headerSearchItem">
                ...
                <span className="headerSearchText" onClick={flipopendate} > {`${format(date[0].startDate, "dd/MM/yyyy")} to ${format(date[0].endDate, "dd/MM/yyyy")}`} </span>
                {opendates && <DateRange
